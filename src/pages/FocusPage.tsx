...
const FocusPage = () => {
  ...
  const status = useAppSelector((state) => state.task.selectedTask?.status)



  const nextTask = tasks.find(task => task.status === status && task.id !== taskId)
  const focusTask = tasks.find(task => task.id === taskId)
  return (
    <>
      <div className="flex bg-bg-primary min-h-screen">
        <div className=" flex-1 p-6 text-center justify-center max-w-300 mx-auto">
          <div className="relative">
            <div className="w-full border border-purple-200 opacity-40 absolute  mt-3" />
            <span className="text-text-primary text-3xl capitalize relative bottom-1.5 bg-bg-primary px-5">Today's Focus</span>
            <div className="bg-bg-secondary p-10 my-5 rounded-2xl mx-70 card-glow">
          ...
          <div className="text-text-primary mt-10 text-left relative">
            <div>
              <div className="w-full border border-purple-200 opacity-40 mt-3 absolute" />
              <span className="relative bg-bg-primary p-5">Next Task</span>
            </div>
            <div className=" mt-10 mx-60">
              <div className="text-left text-4xl py-10 px-5 bg-bg-secondary rounded-2xl">
                {nextTask?.title ?? 'No more new tasks!'}
              </div>
            </div>
            <div className="w-full border border-purple-200 opacity-40 absolute mt-10" />
          </div>
